...
Manuscript status: The contents of this chapter was compiled from a research article published in the journal *Water Resources Research*.

Citation: Cebulski, A. C., & Pomeroy, J. W. (2026). Processes Governing the Ablation of Intercepted Snow. *Water Resources Research*, 62(2), e2025WR042009. https://doi.org/10.1029/2025WR042009
...
The objective of this study is to better understand and predict the influences of meteorology and snow load on intercepted snow ablation. This study specifically examines canopy snow ablation following snowfall and initial interception. @sec-chapter-3 has addressed processes governing the initial interception of snow in the canopy.
...
where $h$ is the liquid meltwater holding capacity that the canopy can retain against gravity (-) and $c$ is a storage constant of the vegetation elements (-). @Andreadis2009 estimated $h$ as 0.035 and $c$ as $e^{-4}$ while using a two sided plant (leaf) area index (LAI). In this study, $h$ was set to 0.01, and $c$ was defined as $C_c \cdot 0.2$ and a one-sided effective LAI was used, including the clumping factor to account for needleleaf canopy structures, following the approach of @Ellis2010 for rainfall interception.
...
where $\rho_{act}$ is the air density (kg m^-3^), $C_p^{air}$ is the specific heat capacity of air (J kg^-1^ K^-1^), $T_a$ is the air temperature, and $r_a$ is the aerodynamic resistance (s m^-1^) which was approximated from Equation 4 in @Allan1998 as:
...
A mass balance approach was incorporated to determine the unloading rate, $q_{unld}^{melt}$ during melt periods. The effect of the canopy snowmelt rate ($q_{melt}^{veg}$) on unloading was then assessed by fitting a linear model using an ordinary least squares regression. Since direct measurements of canopy snow unloading are challenging to obtain independently of canopy snowmelt drainage [@Storck2002], the mass balance introduced in @eq-canopy-mass-bal-04 was incorporated to determine $q_{unld}^{melt}$ as a residual. During intervals without precipitation or $q_{wind}^{veg}$ @eq-canopy-mass-bal-04 was simplified and rearranged to:
...
where $\tau_{mid}$ is the shear stress at mid-canopy height (N m^-2^) and $a$ is a fitting constant (@fig-q-unld-wind).
...
where $m$ and $w$ are fitting coefficients determined as `r q_unld_melt_ratio_m` and `r q_unld_melt_ratio_w`, respectively, using the generalised least squares regression shown in @fig-unld-melt-ratio (*R^2^* = `r q_unld_melt_ratio_r2`, RMSE = `r q_unld_melt_ratio_rmse` (-)). The ordinary least squares regression exhibited heteroscedasticity, with residual variance increasing at higher unloading-to-melt ratios (Breusch-Pagan test *p* < 0.05; @sec-appdx-ch4) and was accounted for by using a generalised least squares regression. See @sec-appdx-ch4 for additional statistical details. 
...
The new model is based on the canopy snow mass balance formulation (@eq-canopy-mass-bal-04), where $q_{tf}$ was represented as:
...
and $C_p$ is the leaf contact area calculated using @eq-lca-ac in @sec-chapter-3 and $\alpha$ is an efficiency constant. 
